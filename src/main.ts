...
async function initializeMarket() {
  ...
  const liquidityCurrencyMint = new anchor.web3.PublicKey(
  ...
  console.log(
    `Lending Market Account Pub ${lendingMarketAccount.publicKey} Secret ${lendingMarketAccount.secretKey}`
  );
...
async function initializeTrove() {
  ...
  const associatedTokenAccount = await utils.findAssociatedTokenAddress(
    owner,
